...
export default function FotoMeal({ mealLabel, onAccept, onClose }) {
  const { user } = useAuth()
  ...
  // Comprimir imagen a max 1024px y JPEG 0.7
      ...
      <div className="foto-meal-modal" onClick={e => e.stopPropagation()}>
        ...
        {items && (
          <div className="foto-meal-results">
                ...
                <tbody>
                  {items.map((it, i) => (
                    <tr key={i}>
                      <td>
                        <input
                          className="foto-meal-cell-input foto-meal-cell-name"
                          value={it.name}
                          onChange={e => updateItem(i, 'name', e.target.value)}
                        />
                      </td>
                      <td><input className="foto-meal-cell-input" type="number" value={it.weight} onChange={e => updateItem(i, 'weight', e.target.value)} inputMode="numeric" /></td>
                      <td><input className="foto-meal-cell-input" type="number" value={it.kcal} onChange={e => updateItem(i, 'kcal', e.target.value)} inputMode="numeric" /></td>
                      <td><input className="foto-meal-cell-input" type="number" value={it.protein} onChange={e => updateItem(i, 'protein', e.target.value)} inputMode="numeric" /></td>
                      <td><input className="foto-meal-cell-input" type="number" value={it.carbs} onChange={e => updateItem(i, 'carbs', e.target.value)} inputMode="numeric" /></td>
                      <td><input className="foto-meal-cell-input" type="number" value={it.fat} onChange={e => updateItem(i, 'fat', e.target.value)} inputMode="numeric" /></td>
            ...
            <button
              className="salud-btn salud-btn-save"
              onClick={() => onAccept(Math.round(totals.kcal))}
            >
